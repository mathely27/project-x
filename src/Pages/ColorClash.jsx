...
export default function ColorClashGame({ username = "Player", onGameComplete }) {
  ...
  const handleAnswer = (selectedColor) => {
    ...
    const isCorrect = selectedColor.name === currentQuestion.visualColor;
    ...
    if (question >= TOTAL_QUESTIONS) {
      ...
      if (onGameComplete) {
        onGameComplete({
          username: username || "Player",
          score: newScore,
          accuracy: Math.round((newCorrectAnswers / TOTAL_QUESTIONS) * 100),
          completionTime: elapsed,
        });
      }
      return;
    ...
    return (
      <div className="cc-game">
        ...
        <div className="cc-intro">
          ...
          <p className="cc-subtitle">Don't read it. See it.</p>
          <div className="cc-example">
            <span className="cc-example-label">EXAMPLE</span>
            <div className="cc-example-word" style={{ color: "#22c55e" }}>BLUE</div>
            <p>Click <strong>GREEN</strong></p>
          </div>
          <button className="cc-start-btn" onClick={startGame}>START GAME</button>
        </div>
      </div>
    );
  }
      ...
      <div className="cc-game">
        ...
        <div className="cc-play-area">
          <div className="cc-top-bar">
            <div>
              <div className="cc-small-heading">COLOR CLASH</div>
              <div className="cc-question">Question {question} / {TOTAL_QUESTIONS}</div>
            </div>
          ...
          <div className="cc-timer-wrapper">
            ...
            <div className="cc-timer-track">
              <div className="cc-timer-fill" style={{ width: `${timerPercentage}%` }} />
            </div>
          </div>
          <div className="cc-center">
            <div className="cc-instruction">CLICK THE <span>TEXT COLOR</span></div>
            <div className="cc-word" style={{ color: currentQuestion.visualColorValue }}>
              {currentQuestion.word}
            </div>
            <div className="cc-options">
              {options.map((color) => (
                <button key={color.name} className="cc-option" onClick={() => handleAnswer(color)}>
                  <span className="cc-option-dot" style={{ backgroundColor: color.value }} />
                  {color.name}
                </button>
              ))}
            </div>
  ...
  return (
    <div className="cc-game">
      ...
      <div className="cc-result">
        <div className="cc-mini-label">GAME COMPLETE</div>
        <h1 className="cc-title">COLOR <span>CLASH</span></h1>
        ...
        <div className="cc-stats">
          <div className="cc-stat">
            <span>ACCURACY</span>
            <strong>{Math.round((correctAnswers / TOTAL_QUESTIONS) * 100)}%</strong>
          </div>
          <div className="cc-stat">
            <span>CORRECT</span>
            <strong>{correctAnswers}/{TOTAL_QUESTIONS}</strong>
          </div>
          <div className="cc-stat">
            <span>TIME</span>
        ...
        <button className="cc-start-btn" onClick={playAgain}>PLAY AGAIN</button>
      </div>
    </div>
  );
}